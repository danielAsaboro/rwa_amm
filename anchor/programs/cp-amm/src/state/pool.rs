...
use crate::{
    ...
    safe_math::SafeMath,
    state::{ fee::{ DynamicFeeStruct, FeeOnAmountResult, PoolFeesStruct }, Position, SplitFeeAmount },
...
/// collect fee mode
#[repr(u8)]
#[derive(Clone, Copy, Debug, PartialEq, IntoPrimitive, TryFromPrimitive, AnchorDeserialize, AnchorSerialize)]
...
impl PoolMetrics {
    ...
    pub fn accumulate_fee(&mut self, lp_fee: u64, protocol_fee: u64, partner_fee: u64, is_token_a: bool) -> Result<()> {
        if is_token_a {
...
impl RewardInfo {
    ...
    pub fn update_rewards(&mut self, liquidity_supply: u128, current_time: u64) -> Result<()> {
        ...
        if self.initialized() {
            if liquidity_supply > 0 {
                let reward_per_token_stored_delta = self.calculate_reward_per_token_stored_since_last_update(
                    current_time,
                    liquidity_supply
                )?;
    ...
    pub fn calculate_reward_per_token_stored_since_last_update(
        ...
        let reward_per_token_stored = shl_div_256(total_reward, liquidity_supply, LIQUIDITY_SCALE).ok_or_else(
    ...
    pub fn accumulate_reward_per_token_stored(&mut self, delta: U256) -> Result<()> {
        self.reward_per_token_stored = self.reward_per_token_stored().safe_add(delta)?.to_le_bytes();
        Ok(())
    }
    ...
    pub fn update_rate_after_funding(&mut self, current_time: u64, funding_amount: u64) -> Result<()> {
        ...
        let total_amount = if current_time >= reward_duration_end {
            ...
            let leftover: u64 = safe_mul_shr_cast(self.reward_rate, remaining_seconds.into(), REWARD_RATE_SCALE)?;
...
impl Pool {
    ...
    pub fn get_swap_result(
        ...
        let SwapAmount { output_amount, next_sqrt_price } = (match trade_direction {
    ...
    pub fn apply_swap_result(
        ...
        let fee_per_token_stored = shl_div_256(lp_fee.into(), self.liquidity, LIQUIDITY_SCALE).ok_or_else(
            || PoolError::MathOverflow
        )?;
        ...
        if fee_mode.fees_on_token_a {
            ...
            self.fee_a_per_liquidity = self.fee_a_per_liquidity().safe_add(fee_per_token_stored)?.to_le_bytes();
            ...
            self.fee_b_per_liquidity = self.fee_b_per_liquidity().safe_add(fee_per_token_stored)?.to_le_bytes();
    ...
    pub fn get_amounts_for_modify_liquidity(
        ...
        // finding output amount
        let token_a_amount = get_delta_amount_a_unsigned(self.sqrt_price, self.sqrt_max_price, liquidity_delta, round)?;

        let token_b_amount = get_delta_amount_b_unsigned(self.sqrt_min_price, self.sqrt_price, liquidity_delta, round)?;
    ...
    pub fn apply_add_liquidity(&mut self, position: &mut Position, liquidity_delta: u128) -> Result<()> {
    ...
    pub fn apply_remove_liquidity(&mut self, position: &mut Position, liquidity_delta: u128) -> Result<()> {
        // update current fee for position
        position.update_fee(self.fee_a_per_liquidity(), self.fee_b_per_liquidity())?;
    ...
    pub fn apply_split_position(
        ...
        if unlocked_liquidity_percentage > 0 {
            let unlocked_liquidity_delta =
                first_position.get_unlocked_liquidity_by_percentage(unlocked_liquidity_percentage)?;
        ...
        if permanent_locked_liquidity_percentage > 0 {
            let permanent_locked_liquidity_delta = first_position.get_permanent_locked_liquidity_by_percentage(
        ...
        if fee_a_percentage > 0 || fee_b_percentage > 0 {
            let SplitFeeAmount { fee_a_amount, fee_b_amount } = first_position.get_pending_fee_by_percentage(
                fee_a_percentage,
                fee_b_percentage
            )?;
        ...
        if self.pool_reward_initialized() {
            if reward_0_percentage > 0 {
                ...
                if pool_reward_info.initialized() {
                    let split_reward = first_position.get_pending_reward_by_percentage(
                        REWARD_INDEX_0,
                        reward_0_percentage
                    )?;
    ...
    pub fn accumulate_permanent_locked_liquidity(&mut self, permanent_locked_liquidity: u128) -> Result<()> {
        self.permanent_lock_liquidity = self.permanent_lock_liquidity.safe_add(permanent_locked_liquidity)?;
    ...
    pub fn claim_partner_fee(&mut self, max_amount_a: u64, max_amount_b: u64) -> Result<(u64, u64)> {
    ...
    pub fn validate_authority_to_edit_reward(&self, reward_index: usize, signer: Pubkey) -> Result<()> {
