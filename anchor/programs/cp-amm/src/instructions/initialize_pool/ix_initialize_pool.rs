...
use anchor_spl::{ token_2022::Token2022, token_interface::{ Mint, TokenAccount, TokenInterface } };
use std::cmp::{ max, min };

use crate::{
    activation_handler::ActivationHandler,
    const_pda,
    constants::seeds::{ POOL_PREFIX, POSITION_NFT_ACCOUNT_PREFIX, POSITION_PREFIX, TOKEN_VAULT_PREFIX },
    create_position_nft,
...
pub struct InitializePoolCtx<'info> {
    ...
    #[account(
        init,
        seeds = [POSITION_PREFIX.as_ref(), position_nft_mint.key().as_ref()],
        bump,
        payer = payer,
    ...
    /// Token a mint
    #[account(constraint = token_a_mint.key() != token_b_mint.key(), mint::token_program = token_a_program)]
    ...
    #[account(
        init,
        seeds = [TOKEN_VAULT_PREFIX.as_ref(), token_a_mint.key().as_ref(), pool.key().as_ref()],
    ...
    #[account(
        init,
        seeds = [TOKEN_VAULT_PREFIX.as_ref(), token_b_mint.key().as_ref(), pool.key().as_ref()],
...
pub fn handle_initialize_pool<'c: 'info, 'info>(
    ...
    let InitializePoolParameters { liquidity, sqrt_price, activation_point } = params;
    ...
    require!(config.get_config_type()? == ConfigType::Static, PoolError::InvalidConfigType);
    ...
    let activation_point = activation_point.unwrap_or(ActivationHandler::get_current_point(config.activation_type)?);

    require!(sqrt_price >= config.sqrt_min_price && sqrt_price <= config.sqrt_max_price, PoolError::InvalidPriceRange);
    ...
    position.initialize(&mut pool, ctx.accounts.pool.key(), ctx.accounts.position_nft_mint.key(), liquidity);
