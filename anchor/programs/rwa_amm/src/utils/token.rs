...
pub fn transfer_from_user<'a, 'c: 'info, 'info>(
    ...
    amount: u64,
    remaining_infos: &[AccountInfo<'info>],
) -> Result<()> {
    ...
    // Forward any remaining accounts to support Token-2022 Transfer Hook ExtraAccountMetaList
    // If no remaining accounts are present, this behaves as a normal transfer
    account_infos.extend_from_slice(remaining_infos);
...
pub fn transfer_from_pool<'c: 'info, 'info>(
    ...
    // Forward remaining accounts for Transfer Hook execution
    account_infos.extend_from_slice(remaining_infos);
...
/// Returns the transfer hook program id if the mint has the TransferHook extension
pub fn get_transfer_hook_program_id<'info>(
    ...
    let token_mint_unpacked =
        StateWithExtensions::<spl_token_2022::state::Mint>::unpack(&token_mint_data)?;
    if let Ok(hook) = token_mint_unpacked.get_extension::<transfer_hook::TransferHook>() {
        let opt: Option<Pubkey> = hook.program_id.into();
        return Ok(opt);
    }
    Ok(None)
}
