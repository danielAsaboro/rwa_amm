...
use crate::{
    ...
    state::{Config, ConfigType, HookWhitelist, Pool, PoolType, Position},
    token::{
        calculate_transfer_fee_included_amount, get_token_program_flags, get_transfer_hook_program_id,
        is_supported_mint, is_token_badge_initialized, transfer_from_user,
    },
...
pub fn handle_initialize_pool<'c: 'info, 'info>(
    ...
    // enforce whitelist if hook present
    ...
    let remaining_infos: Vec<AccountInfo> = ctx.remaining_accounts.iter().cloned().collect();
