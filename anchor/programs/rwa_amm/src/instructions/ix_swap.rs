...
pub fn handle_swap<'info>(ctx: Context<'_, '_, '_, 'info, SwapCtx<'info>>, params: SwapParameters) -> Result<()> {
    {
    ...
    // enforce hook whitelist for input and output mints if present
    if let Some(hook_program) = get_transfer_hook_program_id(token_in_mint)? {
        require!(
            ctx.accounts.hook_whitelist.is_whitelisted(&hook_program),
            PoolError::InvalidInput
        );
    }
    if let Some(hook_program) = get_transfer_hook_program_id(token_out_mint)? {
        require!(
            ctx.accounts.hook_whitelist.is_whitelisted(&hook_program),
            PoolError::InvalidInput
        );
    ...
    // materialize remaining accounts once for all transfers
    let remaining_infos: Vec<AccountInfo> = ctx.remaining_accounts
        .iter()
