...
use crate::{
    ...
    state::{pool::Pool, HookWhitelist}, token::{get_transfer_hook_program_id, transfer_from_pool},
};
...
pub fn handle_withdraw_ineligible_reward<'info>(
    ...
    if ineligible_reward > 0 {
        // enforce hook whitelist if present
        if let Some(hook_program) = get_transfer_hook_program_id(&ctx.accounts.reward_mint)? {
            require!(
